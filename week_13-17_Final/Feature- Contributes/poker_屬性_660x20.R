...
#poker<-readtext("D:/_PROJECT_/Data_Red/*.txt")   #TEST
...
#*** SUMMARY ***分割後取上半部分 ------------
#nosum= list()
game_nosum= unlist(lapply(game, function(x){append(nosum, x[1])}))

nosum_new= unlist(lapply(game, function(x){append(nosum, x[1])}))
nosum_new<-lapply(nosum_new,function(xi)unlist(strsplit(xi,"small blind", fixed = TRUE,useBytes = TRUE)))
nosum_new<-lapply(nosum_new,function(xi)unlist(strsplit(xi,"\n*** TURN ***", fixed = TRUE,useBytes = TRUE)))

#str(game_nosum)

#截出posts small blind、*** FLOP ***、*** TURN *** ------------
game_nosum<-lapply(game_nosum,function(xi)unlist(strsplit(xi,"small blind", fixed = TRUE,useBytes = TRUE)))
#small_blind[1]檢視
...
poker_name <- c( "bigruboss69", "przemol", "sergbest2005", "spadevalet", "pomkaPWNZ", "ChikoKz740", "Donkey84Kong",
                 "PerkeoX", "yanisarais", "daani.p23", "pstnyas", "stare-misto", "One23drumm", "Fabula_16", "Cendman",
                 "Ayohla", "rawmechanic", "MaksCh1", "Foccc", "Posaunengodi")
gamelist=list() 
...
#-----preflop換行切
only_pf= list()
only_pf= unlist(lapply(game_nosum, function(x){append(only_pf, x[2])}))
only_pf= lapply(only_pf,function(xi)unlist(strsplit(xi,"\n*** HOLE CARDS ***\n", fixed = TRUE,useBytes = TRUE)))
only_hc= list()
only_hc= unlist(lapply(only_pf, function(x){append(only_hc, x[2])}))
only_hc= lapply(only_hc,function(xi)unlist(strsplit(xi,'\n',fixed = TRUE,useBytes = TRUE)))
only_hc= unlist(only_hc)
hc_move= list()
hc_raise= list()
head(only_hc)

#Preflop Raise
hc_move= lapply(poker_name, function(q){
  append(hc_move, length(grep(paste0(q, ": "), only_hc))) })
hc_raise= lapply(poker_name,function(q){ 
  append(hc_raise, length(grep(paste0(q, ": raises"), only_hc)))})

#for(q in poker_name){
#  m= length(grep(paste0(q, ": "), only_hc))
#  hc_move= append(hc_move, m)
#  r= length(grep(paste0(q, ": raises"), only_hc))
#  hc_raise= append(hc_raise, r)
#}
...
#preflop加注率
hc_move= unlist(hc_move)
hc_raise= unlist(hc_raise)
PFR= as.vector(hc_raise)/as.vector(hc_move) #PreFlop Raise
...
#屬性DF
Attributes= data.frame(Hfoldlist,gamelist,FtBiPF, hc_raise, hc_move, PFR, Ffoldlist,floplist,FtBiF,F_raises_nb,other_BR_nb,RRiF,
                       cbet2_1list,cbet2list,CBiF)
row.names(Attributes)=  poker_name

colnames(Attributes)= c('PreFlop蓋牌次數','參與PreFlop次數','PreFlop蓋牌率',
                        'PreFlop加注次數', 'Hole Cards動作數', 'Preflop加注率',
...
Attributes
View(Attributes)
#write.csv(Attributes,'屬性660x20.csv')
